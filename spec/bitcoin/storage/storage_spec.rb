...
[
  ...
  describe "Storage::Backends::#{options[0].to_s.capitalize}Store (#{options[1]})" do
    ...
    it "should get head block" do
      block = @store.block("0000000098932356a236718829dd9e3eb0f9143317ab921333b1a203de336de4")
    ...
    it "should get locator" do
      ...
      @store.locator.should == locator
      @store.get_locator.should == locator
    end
    ...
    it "should get block by hash" do
      @store.block("00000007199508e34a9ff81e6ec0c477a4cccff2a4767a8eee39c11db367b008").hash
        .should == P::Block.new(fixtures_file('testnet/block_0.bin')).hash
      @store.block("000000033cc282bc1fa9dcae7a533263fd7fe66490f550d80076433340831604").hash
        .should == P::Block.new(fixtures_file('testnet/block_1.bin')).hash
      @store.block("000000037b21cac5d30fc6fda2581cf7b2612908aed2abbcc429c45b0557a15f").hash
    ...
    unless ['utxo', 'spv'].include?(@store.backend_name)
      describe :transactions do

        it "should get block for tx" do
          @store.store_block(@blk)
          @store.block_by_tx(@blk.tx[0].hash).should == @blk
        end
        ...
        it "should get the position for a given tx" do
          @store.store_block(@blk)
          result = @store.idx_from_tx_hash(@blk.tx[0].hash)
          result.should == 0
        end
    ...
    describe :txouts do
      ...
      it "should get txouts for pk script" do
        ...
        @store.txouts_for_pk_script(script).should == [@blk.tx[0].out[0]]
      end

      it "should get txouts for hash160" do
        @store.txouts_for_hash160(@key2.hash160, :pubkey_hash, true).should == [@block.tx[1].out[0]]
      end

      it "should get txouts for address" do
        @store.txouts_for_address(@key2.addr, true).should == [@block.tx[1].out[0]]
      end
      ...
      it "should get unspent txouts for address" do
        @store.unspent_txouts_for_address(@key2.addr, true).should == [@block.tx[1].out[0]]
        @block2 = create_block @block.hash, true, [->(t) {
           create_tx(t, @block.tx[1], 0, [[20, @key2]], @key2) }], @key
        @store.unspent_txouts_for_address(@key2.addr, true).should == [@block2.tx[1].out[0]]
      end
