  ...
  class Block
    ...
    def min_timestamp
      ...
      if ["sequel", "utxo"].include?(store.backend_name)
        ...
        times = [first.time]
        (10).times { first = first.prev_block; times << first.time }
      end
      times.sort!
    ...
    def prev_txs_hash
      @prev_tx_hash ||= (
        ...
        txs = store.txs(inputs.map{|i| i.prev_out_hash.reverse_hth })
  ...
  class Tx
    ...
    def prev_txs
      @prev_txs ||= tx.in.map {|i|
        prev_tx = block_validator ? block_validator.prev_txs_hash[i.prev_out_hash.reverse_hth] : store.tx(i.prev_out.reverse_hth)
