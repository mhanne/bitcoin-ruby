  ...
  class UtxoStore < StoreBase
    ...
    def persist_transactions txs, block_id, depth
      txs.each.with_index do |tx, tx_blk_idx|
        ...
        tx.out.each.with_index do |txout, txout_tx_idx|
          ...
          @new_outs << [{
              :tx_hash => tx.hash.to_sequel_blob,
              :tx_idx => txout_tx_idx,
              :blk_id => block_id,
    ...
    def reorg new_side, new_main
      new_side.each do |block_hash|
        ...
        depth = get_depth
        blk = @db[:blk][hash: block_hash.htb.to_sequel_blob]
      ...
      new_main.each do |block_hash|
        block = @db[:blk][hash: block_hash.htb.to_sequel_blob]
    ...
    def has_tx(tx_hash)
      !!@db[:utxo].where(:hash => tx_hash.to_sequel_blob).get(1)
    end
    ...
    def get_block_by_tx(tx_hash)
      block_id = @db[:utxo][tx_hash: tx_hash.to_sequel_blob][:blk_id]
    ...
    def get_txout_for_txin(txin)
      wrap_txout(@db[:utxo][tx_hash: txin.prev_out.reverse.hth.to_sequel_blob, tx_idx: txin.prev_out_index])
    end
    ...
    def wrap_tx(tx_hash)
      utxos = @db[:utxo].where(tx_hash: tx_hash.to_sequel_blob)
